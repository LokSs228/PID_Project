def CHR_0(process_gain, time_constant, delay_time):
    if process_gain == 0 or time_constant <= 0 or delay_time <= 0:
        return None

    p_kp = 0.3 * time_constant / (process_gain * delay_time)

    pi_kp = 0.35 * time_constant / (process_gain * delay_time)
    pi_ki = pi_kp / (1.2 * time_constant)

    pid_kp = 0.6 * time_constant / (process_gain * delay_time)
    pid_ki = pid_kp / time_constant
